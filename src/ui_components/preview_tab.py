...
def build_preview_tab(
    preview_content: Dict[str, str], update_callback: Callable
) -> None:
    ...
    with ui.row().classes("w-full items-center mb-4"):
        ...
        ui.space()
        ui.button(
            "Обновить", on_click=update_callback, icon="refresh", color="primary"
        ).props("outline")
    ...
    **Инструкция:** Здесь отображается итоговая конфигурация в формате YAML. 
    ...
    with code_container.style("height: calc(100vh - 300px)"):
        ...
        yaml_code.bind_content_from(
            preview_content,
            "yaml",
            backward=lambda x: x or "# Нажмите 'Обновить' для генерации предпросмотра",
        )
    ...
    with ui.row().classes("w-full mt-4 gap-4"):
        with ui.card().classes("flex-1"):
            ...
            stats_label.bind_text_from(
                preview_content, "yaml", backward=lambda x: get_stats()
            )
